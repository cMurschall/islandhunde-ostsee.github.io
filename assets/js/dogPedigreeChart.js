(() => {
  ...
  // assets/js/dogPedigreeChartBundle.js
  var generateUniqueId = /* @__PURE__ */ (() => {
    let counter = 0;
  ...
  function renderDogPedigreeChart(containerId, dogData) {
    ...
    innerDiv.setAttribute("aria-label", "Stammbaumdiagramm f\xFCr Islandhund " + dogData.name_and_origin);
    ...
    const getOrigin = (dog) => {
      var _a;
      const origin = (_a = dog.id) == null ? void 0 : _a.substring(0, 2).toLowerCase();
      return origin != null ? origin : "is";
    };
    ...
    const svg = select_default2("#" + pedigreeId).append("svg").attr("viewBox", `0 0 ${imageWidth} ${imageHeight}`).attr("preserveAspectRatio", "xMinYMin meet").append("g").attr("transform", `translate(${boxWidth * 0.5},${imageHeight / 2})`);
    const root2 = hierarchy(dogData, (d) => d.parents);
    const treeLayout = tree_default().nodeSize([boxHeight, boxWidth]).separation((a, b) => a.parent === b.parent ? 0.7 : 0.5);
    const treeData = treeLayout(root2);
    svg.selectAll("path.link").data(treeData.links()).join("path").attr("class", "link").attr("d", elbow);
    const node = svg.selectAll("g.dog").data(treeData.descendants()).join("g").attr("class", "dog").attr("transform", (d) => `translate(${d.y},${d.x})`);
    node.append("text").attr("x", -(boxWidth / 2.3 - flagwidth * 1.15)).attr("y", -flagheight * 0.25).attr("text-anchor", "start").attr("style", `font-size:${textSize}px`).attr("class", "name").text((d) => d.data.name_and_origin);
    node.append("image").attr("x", -(boxWidth / 2.3)).attr("y", -flagheight).attr("height", flagheight).attr("width", flagwidth).attr("href", (d) => {
      return `https://raw.githubusercontent.com/lipis/flag-icons/refs/heads/main/flags/4x3/${getOrigin(d.data)}.svg`;
    });
    node.append("text").attr("dx", -(boxWidth / 2.3)).attr("dy", flagheight * 0.8).attr("text-anchor", "start").attr("style", `font-size:${textSize}px`).attr("class", "id").text((d) => isRoot(d.data) ? "" : d.data.id);
    node.append("text").attr("dx", -(boxWidth / 2.3)).attr("dy", flagheight * 0.8 + textSize * 1.2).attr("text-anchor", "start").attr("style", `font-size:${textSize * 0.9}px; fill: #555`).attr("class", "hd").text((d) => d.data.hd ? `HD: ${d.data.hd}` : "");
  }
  var dogPedigreeChartBundle_default = renderDogPedigreeChart;
